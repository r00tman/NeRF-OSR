...
def create_nerf(rank, args):
    ...
    splits = [x.strip() for x in args.render_splits.strip().split(',')]
    assert(len(splits) == 1)
    split = splits[0]
    ...
    for m in range(models['cascade_level']):
        ...
        if args.optim_autoexpo or True:  # todo: upstream
            ...
            f = os.path.join(args.basedir, args.expname, f'coeff_{split}_images.json')
            with open(f) as file:
    ...
    if (args.ckpt_path is not None) and (os.path.isfile(args.ckpt_path)):
        ...
        ckpts = [os.path.join(args.basedir, args.expname, f)
                 for f in sorted(os.listdir(os.path.join(args.basedir, args.expname)))
                                 if f.startswith(f'coeff_{split}_model') and f.endswith('.pth')]
        remove_env = False
        if len(ckpts) == 0:
            logger.info('can\'t find split model, opting for a raw model')
            ckpts = [os.path.join(args.basedir, args.expname, f)
                     for f in sorted(os.listdir(os.path.join(args.basedir, args.expname)))
                     if f.startswith(f'model') and f.endswith('.pth')]
            remove_env = True
    ...
    assert(len(ckpts) > 0 and not args.no_reload)
    if len(ckpts) > 0 and not args.no_reload:
        ...
        for m in range(models['cascade_level']):
            for name in ['net_{}'.format(m), 'optim_{}'.format(m)]:
                ...
                if remove_env:
                    to_remove = []
                    for k in to_load[name]:
                        if 'module.env_params' in k:
                            logger.info(f'oh cool, we found stuff {name} {k}')
                            to_remove.append(k)
                    for k in to_remove:
                        del to_load[name][k]
                ...
                if 'optim' in name and not remove_env:  # only load optim when we don't change images
                    models[name].load_state_dict(to_load[name])

                if 'optim' not in name:
                    # load model. check that only environment might be different
                    missing, unexpected = models[name].load_state_dict(to_load[name], strict=False)
                    assert(len(unexpected) == 0)
                    for k in missing:
                        assert('module.env_params' in k)
                    # disable model gradients, leave only environment coeffs learnable
                    for k, param in models[name].named_parameters():
                        if 'module.env_params' in k:
                            logger.info('bingo, twice found')
                        else:
...
def ddp_train_nerf(rank, args):
    ...
    if rank == 0:
        ...
        f = os.path.join(args.basedir, args.expname, f'coeff_{split}_args.txt')
        with open(f, 'w') as file:
            for arg in sorted(vars(args)):
                attr = getattr(args, arg)
                file.write('{} = {}\n'.format(arg, attr))
        if args.config is not None:
            f = os.path.join(args.basedir, args.expname, f'coeff_{split}_config.txt')
            with open(f, 'w') as file:
                file.write(open(args.config, 'r').read())
    torch.distributed.barrier()

    ray_samplers = load_data_split(args.datadir, args.scene, split=split,
                                   try_load_min_depth=args.load_min_depth)
    val_ray_samplers = load_data_split(args.datadir, args.scene, split=split,
    ...
    if rank == 0:
        writer = SummaryWriter(os.path.join(args.basedir, args.expname+'_coeff_'+split))
    ...
    for global_step in range(start + 1, start + 1 + args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            if 'autoexpo' in ret:
                ...
                loss = rgb_loss + (shadow_reg * 0.01) * np.clip((global_step - 10000) / 20000, 0, 1)
        ...
        if global_step % args.i_img == 0 or global_step == start + 1:
            ...
            if rank == 0:  # only main process should do this
                logger.info('Rendering a random training view')
            time0 = time.time()
        ...
        if rank == 0 and (global_step % args.i_weights == 0 and global_step > 0):
            ...
            fpath = os.path.join(args.basedir, args.expname, f'coeff_{split}_model_{global_step:06d}.pth')
            to_save = OrderedDict()
            for m in range(models['cascade_level']):
                name = 'net_{}'.format(m)
